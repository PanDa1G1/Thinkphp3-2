...
class ParseTemplateBehavior {
    ...
    public function run(&$_data){
        $engine             =   strtolower(C('TMPL_ENGINE_TYPE'));//默认模板：think
        $_content           =   empty($_data['content'])?$_data['file']:$_data['content'];//传入的内容
        $_data['prefix']    =   !empty($_data['prefix'])?$_data['prefix']:C('TMPL_CACHE_PREFIX');//前缀
        if('think'==$engine){
            if((!empty($_data['content']) && $this->checkContentCache($_data['content'],$_data['prefix'])) 
                ||  $this->checkCache($_data['file'],$_data['prefix'])) { // 缓存有效
                //载入模版缓存文件，缓存文件中有html代码
     ...
     */
    /*
      检查指定的文件是否存在 存在则返回true 不存在则返回false
      路径：./Shop/Runtime/Cache/Home/
   */
