...
function I($name,$default='',$filter=null,$datas=null) {
    ...
    if(''==$name) { // 获取全部变量
        ...
        $data       =   $input[$name];
        $filters    =   isset($filter)?$filter:C('DEFAULT_FILTER');//没有指定过滤函数的话，默认为htmlspecialchars
        if($filters) {
            if(is_string($filters)){
                if(0 === strpos($filters,'/')){
...
function U($url='',$vars='',$suffix=true,$domain=false) {
    ...
    if(isset($info['fragment'])) { // 解析锚点
        ...
        if(false !== strpos($anchor,'?')) { // 解析参数 如果锚点后面有?加参数，将其添加到query中
            list($anchor,$info['query']) = explode('?',$anchor,2);
        }        
        if(false !== strpos($anchor,'@')) { // 解析域名 如果有 @ + 域名。将其赋值给host
    ...
    if(isset($host)) {
        ...
        //将$_SERVER['HTTP_HOST'] 第一个'.'之后的内容取出来和 $host 拼接在一起
    }elseif($domain===true){
        $domain = $_SERVER['HTTP_HOST'];
        if(C('APP_SUB_DOMAIN_DEPLOY') ) { // 开启子域名部署
            $domain = $domain=='localhost'?'localhost':'www'.strstr($_SERVER['HTTP_HOST'],'.');//不是localhost 主域名换成www
            // '子域名'=>array('模块[/控制器]');
    ...
    // URL组装
    $depr       =   C('URL_PATHINFO_DEPR');// '/'
    $urlCase    =   C('URL_CASE_INSENSITIVE');// 是否区分大小写
    if($url) {
        if(0=== strpos($url,'/')) {// 定义路由  判断是否以/ 开头
            $route      =   true;
            $url        =   substr($url,1); //取'/' 之后的数据
            if('/' != $depr) {
                $url    =   str_replace('/',$depr,$url);//如果默认的分割符不是'/'需要把url 里面的 '/'全部换成默认的分割符
            }
        }else{//不定义路由
            if('/' != $depr) { // 安全替换
            ...
            // 解析模块、控制器和操作
            $url        =   trim($url,$depr); //删除 $url两侧的 $depr(分割符)
            $path       =   explode($depr,$url);//使用分割符将url拆分
            $var        =   array();
            $varModule      =   C('VAR_MODULE');//默认m
            $varController  =   C('VAR_CONTROLLER');//默认c
            $varAction      =   C('VAR_ACTION');//默认a
            $var[$varAction]       =   !empty($path)?array_pop($path):ACTION_NAME;//如果不为空 取$path数组的最后一个元素。
...
function to_guid_string($mix) {
    if (is_object($mix)) {//  判断是否为一个对象
        return spl_object_hash($mix);//spl_object_hash — 返回指定对象的hash id
    } elseif (is_resource($mix)) {//判断是否是一个资源
        $mix = get_resource_type($mix) . strval($mix);//返回资源的类型
    } else {
        $mix = serialize($mix);
    }
    return md5($mix);
...
function session($name='',$value='') {
    ...
    if(is_array($name)) { // session初始化 在session_start 之前调用
        ...
        if(isset($name['expire']))          {
            ini_set('session.gc_maxlifetime',   $name['expire']);//指定过了多少秒之后数据就会被视为“垃圾”并被清除。 垃圾搜集可能会在 session 启动的时候开始,默认1440
            ini_set('session.cookie_lifetime',  $name['expire']);//以秒数指定了发送到浏览器的 cookie 的生命周期。值为 0 表示“直到关闭浏览器”。默认为 0。参见
        }
        if(isset($name['use_trans_sid']))   ini_set('session.use_trans_sid', $name['use_trans_sid']?1:0);
        if(isset($name['use_cookies']))     ini_set('session.use_cookies', $name['use_cookies']?1:0);//指定是否在客户端用 cookie 来存放会话 ID。默认为 1（启用）
    ...
    if ('' === $value) {
        if(isset($_COOKIE[$name])){
            ...
            if(0===strpos($value,'think:')){
                $value  =   substr($value,6);// 获取think:之后的值
